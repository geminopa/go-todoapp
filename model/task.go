...
// 関数　AddTaskは引数がstring型のnameで、戻り値はTaskのポインターとerror型である(*1)
func AddTask(name string) (*Task, error) {
	...
	// ID,Name,Finishedにid,name,falseを代入したTask型のtaskを定義
	task := Task{
		ID:       id,
		Name:     name,
		Finished: false,
...
// 関数ChangeFinishedTaskの引数はuuid.UUID型のtaskIDで、戻り値はerror型である
func ChangeFinishedTask(taskID uuid.UUID) error {

	// DBのTaskテーブルからtaskIDと一致するidを探し、そのFinishedをtrueにする(*3)
